...
| 授权管理/百度登录 | `apps/web/src/pages/AuthPage.vue` | `apps/web/src/services/auth.api.ts`, `apps/web/src/services/auth-session.ts`, `apps/web/src/stores/auth.store.ts` | `apps/worker/src/routes/auth.routes.ts` | `apps/worker/src/services/baidu/baidu-auth.service.ts`, `apps/worker/src/services/baidu/baidu-token.service.ts` | `users`, `baidu_tokens` | `GET /api/auth/baidu/status`, `GET /api/auth/baidu/login`, `GET /api/auth/baidu/callback`, `GET /api/auth/me`。首次登录不需要手动用户 ID，callback 后调用百度账号资料接口写入真实百度 openid/账号标识。 |
...
| `/api/import` | `apps/worker/src/routes/import.routes.ts` | 自动同步任务、同步任务查询、失败同步文件重试、导入预览、TXT 导入、导入任务查询 |
